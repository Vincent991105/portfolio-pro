function FirstEar({data}){
    return(
        <svg xmlns="http://www.w3.org/2000/svg" width="171" height="96" viewBox="0 0 171 96" fill="none">
            <defs>
                <pattern
                    id="pattern0_344_267"
                    patternContentUnits="objectBoundingBox"
                    width="1"
                    height="1"
                >
                    <image
                        href="data:image/png;base64,"
                        x="0"
                        y="0"
                        width="75"
                        height="75"
                        preserveAspectRatio="none"
                        transform="matrix(0.0133333 0 0 0.0747566 0 -2.30337)"
                        crossOrigin="anonymous"
                    />
                </pattern>
                <pattern
                    id="grey_army"
                    patternContentUnits="objectBoundingBox"
                    width="1"
                    height="1"
                >
                    <image
                        href="data:image/png;base64,"
                        x="0"
                        y="0"
                        width="75"
                        height="75"
                        preserveAspectRatio="none"
                        transform="matrix(0.0133333 0 0 0.0747566 0 -2.30337)"
                        crossOrigin="anonymous"
                    />
                </pattern>
            </defs>
            <path d="M53 3V75.5V93H3V28C3 8 36.3333 3 53 3Z" fill={data?.earHalf} stroke={data?.earOutside} strokeWidth="5"/>
            <path d="M118 3V75.5V93H168V28C168 8 134.667 3 118 3Z" fill={data?.earHalf} stroke={data?.earOutside} strokeWidth="5"/>
            <path d="M123 28V60.2222V68H153V39.1111C153 30.2222 133 28 123 28Z" fill={data?.earInside} stroke={data?.earInside} strokeWidth="5"/>
            <path d="M48 28V60.2222V68H18V39.1111C18 30.2222 38 28 48 28Z" fill={data?.earInside} stroke={data?.earInside} strokeWidth="5"/>
        </svg>
    )
}

export default FirstEar

