function SevenSide({data}){
    return(
        <svg xmlns="http://www.w3.org/2000/svg" width="250" height="76" viewBox="0 0 250 76" fill="none">
            <g clipPath="url(#clip0_44_232)">
                <path d="M37.4995 0.5C16.7891 0.500261 -0.000488281 17.2895 -0.000488281 38C-0.000488281 58.7107 16.7898 75.5 37.5005 75.5H50.0005C41.7162 75.5 34.9995 68.7843 34.9995 60.5V15.5C34.9995 7.21589 41.7155 0.500261 49.9995 0.5H37.4995Z" fill={data?.side}/>
                <path d="M212.5 0.5C233.211 0.500261 250 17.2895 250 38C250 58.7107 233.21 75.5 212.5 75.5H200C208.284 75.5 215 68.7843 215 60.5V15.5C215 7.21589 208.285 0.500261 200 0.5H212.5Z" fill={data?.side}/>
            </g>
            <defs>
                <pattern
                    id="pattern0_344_267"
                    patternContentUnits="objectBoundingBox"
                    width="1"
                    height="1"
                >
                    <image
                        href="data:image/png;base64,"
                        x="0"
                        y="0"
                        width="75"
                        height="75"
                        preserveAspectRatio="none"
                        transform="matrix(0.0133333 0 0 0.0747566 0 -2.30337)"
                        crossOrigin="anonymous"
                    />
                </pattern>
                <pattern
                    id="grey_army"
                    patternContentUnits="objectBoundingBox"
                    width="1"
                    height="1"
                >
                    <image
                        href="data:image/png;base64,"
                        x="0"
                        y="0"
                        width="75"
                        height="75"
                        preserveAspectRatio="none"
                        transform="matrix(0.0133333 0 0 0.0747566 0 -2.30337)"
                        crossOrigin="anonymous"
                    />
                </pattern>
            </defs>
            <defs>
                <clipPath id="clip0_44_232">
                <rect width="250" height="75" fill="white" transform="translate(0 0.5)"/>
                </clipPath>
            </defs>
        </svg>
    )
}

export default SevenSide