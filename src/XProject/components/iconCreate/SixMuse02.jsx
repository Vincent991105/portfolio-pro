function SixMuse02({data}){
    return(
        <svg xmlns="http://www.w3.org/2000/svg" width="145" height="23" viewBox="0 0 145 23" fill="none">
            <g clipPath="url(#clip0_44_210)">
                <path d="M15 3C15 1.61929 13.8807 0.5 12.5 0.5C11.1193 0.5 10 1.61929 10 3C10 4.38071 11.1193 5.5 12.5 5.5C13.8807 5.5 15 4.38071 15 3Z" fill={data?.muse}/>
                <path d="M15 20C15 18.6193 13.8807 17.5 12.5 17.5C11.1193 17.5 10 18.6193 10 20C10 21.3807 11.1193 22.5 12.5 22.5C13.8807 22.5 15 21.3807 15 20Z" fill={data?.muse}/>
                <path d="M5 11C5 9.61929 3.88071 8.5 2.5 8.5C1.11929 8.5 0 9.61929 0 11C0 12.3807 1.11929 13.5 2.5 13.5C3.88071 13.5 5 12.3807 5 11Z" fill={data?.muse}/>
                <path d="M130 3C130 1.61929 131.119 0.5 132.5 0.5C133.881 0.5 135 1.61929 135 3C135 4.38071 133.881 5.5 132.5 5.5C131.119 5.5 130 4.38071 130 3Z" fill={data?.muse}/>
                <path d="M130 20C130 18.6193 131.119 17.5 132.5 17.5C133.881 17.5 135 18.6193 135 20C135 21.3807 133.881 22.5 132.5 22.5C131.119 22.5 130 21.3807 130 20Z" fill={data?.muse}/>
                <path d="M140 11C140 9.61929 141.119 8.5 142.5 8.5C143.881 8.5 145 9.61929 145 11C145 12.3807 143.881 13.5 142.5 13.5C141.119 13.5 140 12.3807 140 11Z" fill={data?.muse}/>
            </g>
            <defs>
                <pattern
                    id="pattern0_344_267"
                    patternContentUnits="objectBoundingBox"
                    width="1"
                    height="1"
                >
                    <image
                        href="data:image/png;base64,"
                        x="0"
                        y="0"
                        width="75"
                        height="75"
                        preserveAspectRatio="none"
                        transform="matrix(0.0133333 0 0 0.0747566 0 -2.30337)"
                        crossOrigin="anonymous"
                    />
                </pattern>
                <pattern
                    id="grey_army"
                    patternContentUnits="objectBoundingBox"
                    width="1"
                    height="1"
                >
                    <image
                        href="data:image/png;base64,"
                        x="0"
                        y="0"
                        width="75"
                        height="75"
                        preserveAspectRatio="none"
                        transform="matrix(0.0133333 0 0 0.0747566 0 -2.30337)"
                        crossOrigin="anonymous"
                    />
                </pattern>
            </defs>
            <defs>
                <clipPath id="clip0_44_210">
                    <rect width="145" height="22" fill="white" transform="translate(0 0.5)"/>
                </clipPath>
            </defs>
        </svg>
    )
}

export default SixMuse02