function ThirdEyebrow02({data}){
    return(
        <svg xmlns="http://www.w3.org/2000/svg" width="179" height="15" viewBox="0 0 179 15" fill="none">
            <defs>
                <pattern
                    id="pattern0_344_267"
                    patternContentUnits="objectBoundingBox"
                    width="1"
                    height="1"
                >
                    <image
                        href="data:image/png;base64,"
                        x="0"
                        y="0"
                        width="75"
                        height="75"
                        preserveAspectRatio="none"
                        transform="matrix(0.0133333 0 0 0.0747566 0 -2.30337)"
                        crossOrigin="anonymous"
                    />
                </pattern>
                <pattern
                    id="grey_army"
                    patternContentUnits="objectBoundingBox"
                    width="1"
                    height="1"
                >
                    <image
                        href="data:image/png;base64,"
                        x="0"
                        y="0"
                        width="75"
                        height="75"
                        preserveAspectRatio="none"
                        transform="matrix(0.0133333 0 0 0.0747566 0 -2.30337)"
                        crossOrigin="anonymous"
                    />
                </pattern>
            </defs>
            <path d="M19.25 0.00585938C19.333 0.00313974 19.4163 0 19.5 0H48.25L64.25 15H19.5C19.4163 15 19.333 14.9959 19.25 14.9932V15H2.75L0.25 13.6562L8.33105 11.6396L2.75 8.63867L6.25 7.76562L0.25 6.26855L12.3125 3.25879L6.25 0H19.25V0.00585938ZM160 0.00585938V0H173L166.938 3.25879L179 6.26855L173 7.76562L176.5 8.63867L170.919 11.6396L179 13.6562L176.5 15H160V14.9932C159.917 14.9959 159.834 15 159.75 15H115L131 0H159.75C159.834 0 159.917 0.00313974 160 0.00585938Z" fill={data?.eyeBrow}/>
        </svg>
    )
}

export default ThirdEyebrow02